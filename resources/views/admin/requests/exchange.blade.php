        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Exchange Requests</h5>
                    <div>
                        ...
                        <table class="table table-bordered" id="exchangeRequestsTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $req->oldEquipment->name ?? 'N/A' }}<br><small>{{ $req->oldEquipment->serial_number ?? '' }}</small></td>
                                    <td>{{ $req->requestedEquipment->name ?? 'N/A' }}<br><small>{{ $req->requestedEquipment->serial_number ?? '' }}</small></td>
                                    <td>{{ Str::limit($req->exchange_reason, 40) }}</td>
                                    <td>
                                        @if($req->has_damage)
                                    ...
                                    <td>
                                        @if($req->status == 'Pending')
                                            <div class="btn-group" role="group">
                                                <form action="{{ route('admin.requests.exchange.approve', $req->id) }}" method="POST" style="display:inline-block">
                                                    @csrf
                                        ...
                                        @elseif($req->status == 'Approved')
                                            <form action="{{ route('admin.requests.exchange.process', $req->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-primary" onclick="return confirm('Process this exchange?')">Process Exchange</button>
                                            </form>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                ...
                                <div class="modal fade" id="rejectModal{{ $req->id }}" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form action="{{ route('admin.requests.exchange.reject', $req->id) }}" method="POST">
                                                @csrf
                                                <div class="modal-header bg-danger text-white">
                                                    <h5 class="modal-title">Reject Exchange Request #{{ $req->id }}</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <p><strong>Employee:</strong> {{ $req->user->name ?? 'N/A' }}</p>
                                                    <p><strong>Old Equipment:</strong> {{ $req->oldEquipment->name ?? 'N/A' }}</p>
                                                    <p><strong>Requested Equipment:</strong> {{ $req->requestedEquipment->name ?? 'N/A' }}</p>
                                                    <div class="mb-3">
                                                        <label class="form-label">Reason for Rejection <span class="text-danger">*</span></label>
                                                        <textarea name="rejection_message" class="form-control" rows="4" required 
                                                            placeholder="Please explain why this exchange request is being rejected..."></textarea>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header bg-info text-white">
                                                <h5 class="modal-title">Rejection Message for Exchange Request #{{ $req->id }}</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p><strong>Employee:</strong> {{ $req->user->name ?? 'N/A' }}</p>
                                ...
                                @empty
                                <tr>
                                    <td colspan="10" class="text-center">No exchange requests found</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
